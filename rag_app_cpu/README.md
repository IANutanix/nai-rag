...
- **CPU-Only Mode**: Toggle for CPU-optimized inference
- **Endpoint URL**: OpenAI-compatible API endpoint
- **Model Name**: Model to use for responses  
- **API Key**: Your API authentication key

Default configuration uses Nutanix AI endpoint with llama3370b model (GPU) or gemma-9b-cpu model (CPU mode).

### CPU Mode

Enable CPU-Only Mode for:
- 🖥️ CPU-optimized inference with gemma-9b-cpu model
- ⚡ Reduced token limits (256 vs 5000) for efficiency
- 🎯 Lower temperature (0.3) for more focused responses
- 💾 Reduced memory and compute requirements

## Usage

1. ⚙️ Configure CPU mode if needed (toggle in sidebar)
2. 🧪 Test your API connection using the "Test Connection" button
3. 💬 Start chatting with the documents using the chat input
4. 🔍 The system searches relevant documents and generates contextual responses
